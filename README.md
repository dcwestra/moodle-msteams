...
The older "ends on date" option was removed in 1.7.2, because the resulting number of sessions wasn't visible from the form. Existing meetings that used an end date are converted to the equivalent occurrence count automatically on upgrade.

Saving a recurring activity is idempotent: occurrence rows are reconciled against the schedule rather than rebuilt, so saving any number of times produces exactly one row per session. Editing a series keeps attendance history for sessions that have already run, and keeps existing calendar invites for sessions whose time hasn't changed. (Before 1.7.2, re-saving appended a duplicate row for every occurrence that had already started.)
...
### Editing a Meeting
Saving the activity only contacts Microsoft when the edit actually affects the meeting. Changing the name, start or end time, lobby setting, auto-record, recurrence, or the co-organiser list triggers a sync. Changing anything that is purely LMS-side — completion thresholds, recording mode, the description, grade or visibility settings — does not.

This matters because syncing cancels and recreates the organiser's calendar event, which sends every co-organiser a cancellation followed by a fresh invite. Before 1.7.3 that happened on every save, so adjusting a completion percentage alarmed people who had nothing to do with the change.
...
| 1.7.3 | **Editing an activity no longer disturbs the meeting unless it needs to.** Saving used to re-sync with Microsoft every time, which cancels and recreates the organiser's calendar event and mails every co-organiser a cancellation plus a new invite — even when the edit only changed something like a completion threshold. The plugin now compares what changed and only contacts Microsoft when the meeting itself is affected. Also adds double-submit protection to the activity form, so a stray double-click can't create two Teams meetings. |
| 1.7.2 | **Duplicate occurrence rows on re-save fixed.** Re-saving a recurring activity appended another full copy of every occurrence that had already started. Occurrence rows are now reconciled rather than rebuilt, so saving is idempotent. The "ends on date" recurrence option was removed in favour of an occurrence count, because the resulting number of sessions wasn't visible from the form (existing meetings are converted automatically). Upgrade de-duplicates existing occurrence rows without touching any that carry attendance data. |
